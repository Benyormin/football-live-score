...
final class GridModel<K> {
    ...
    GridModel(GridHost<K> host, ItemKeyProvider<K> keyProvider, SelectionTracker.SelectionPredicate<K> selectionPredicate) {
        ...
        AnonymousClass1 r0 = new RecyclerView.OnScrollListener() {
    ...
    private boolean isPossiblePositionNearestOrigin(int columnIndex, int columnStartIndex, int columnEndIndex, int rowIndex, int rowStartIndex, int rowEndIndex) {
        switch (computeCornerNearestOrigin()) {
            case 0:
                if (columnIndex == columnStartIndex && rowIndex == rowStartIndex) {
                    return true;
                }
                return false;
            case 1:
                if (columnIndex == columnStartIndex && rowIndex == rowEndIndex) {
                    return true;
                }
                return false;
            case 2:
                if (columnIndex == columnEndIndex && rowIndex == rowStartIndex) {
                    return true;
                }
                return false;
            case 3:
    ...
    private static class RelativeCoordinate implements Comparable<RelativeCoordinate> {
        ...
        RelativeCoordinate(List<Limits> limitsList, int value) {
            ...
            if (index >= 0) {
            ...
            } else if ((index ^ -1) == limitsList.size()) {
                ...
                Limits limitsBeforeIndex = limitsList.get((index ^ -1) - 1);
                if (limitsBeforeIndex.lowerLimit > value || value > limitsBeforeIndex.upperLimit) {
                    this.type = 2;
                    this.limitsBeforeCoordinate = limitsList.get((index ^ -1) - 1);
                    this.limitsAfterCoordinate = limitsList.get(index ^ -1);
                    return;
                }
                this.type = 3;
                this.limitsBeforeCoordinate = limitsList.get((index ^ -1) - 1);
    ...
    private static class RelativePoint {
        ...
        public boolean equals(Object other) {
            ...
            if (!this.mX.equals(otherPoint.mX) || !this.mY.equals(otherPoint.mY)) {
                return false;
            }
            return true;
        }
    ...
    private boolean areItemsCoveredByBand(RelativePoint first, RelativePoint second) {
        return doesCoordinateLocationCoverItems(first.mX, second.mX) && doesCoordinateLocationCoverItems(first.mY, second.mY);
    }
